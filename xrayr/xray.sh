...
if [[ $Install_status != "1" ]]; then
        read -erp " 当前系统未识别的XrayR主程序，是否安装（Y/n）:" Install_status_Yn
          ...
          read -erp " 如果主程序启用DNS解锁，请输入DNS解锁地址(回车跳过则主程序无DNS解锁):" dns_nf          read -erp " 是否修改系统DNS为8.8.8.8,如果是Linode等IPv6机器请务必修改（默认Y）:" Install_DNS_NAME
...
echo "当前已安装主程序，请直接对接节点"
        ...
        read -erp " 现在开始对接节点(cir|dog|cv|fq|tbs|hx|rm|dns):" node_web
        if [[ $node_web == "cir" ]]; then
    node_web="circlecloud123.com"
        node_type="V2ray"
        node_panel="V2board"
        node_key="Ming@yyds123456789"
    elif [[ $node_web == "" ]]; then
        node_web=""
        node_type=""
        node_panel=""
